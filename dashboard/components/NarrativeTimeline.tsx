...
export function NarrativeTimeline({ pair, demandLabel, basketLabel }: Props) {
  if (!pair) {
    return (
  ...
  const { weekly_type, weekly_lag, weekly_corr, phases } = pair;

  // Use pre-computed phases if available, otherwise auto-segment
  const computedPhases =
    phases && phases.length > 0
      ? phases
      : autoSegment(weekly_type, weekly_lag, weekly_corr as number[]);
      ...
      {computedPhases.map((p, i) => {
        ...
        return (
          ...
          >
            ...
            <span className="inline-block rounded px-1.5 py-0.5 text-[11px] bg-slate-800/60 text-slate-400">
              r = {p.avg_corr.toFixed(3)}
            </span>
